      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <b
              className={`mt-1 inline-block rounded px-1.5 py-0.5 text-[10px] capitalize ${
...
const AdminPlayersView = () => {
  ...
  const loadPlayers = async () => {
    try {
      ...
      setError(
        err.message || "Unable to load players."
      );
    } finally {
      setLoading(false);
    }
  ...
  const hasFilters =
    search || session || category || status || team;
  ...
  const deletePlayer = async (player) => {
    ...
    try {
      // IMPORTANT:
      // DELETE request goes to /api/players
      // NOT /api/players/[id]
      const response = await fetch(
        "/api/players",
        {
      ...
      // Remove player immediately from UI
      setPlayers((current) =>
        current.filter(
          (currentPlayer) =>
      ...
      // Close modal if deleted player was selected
  ...
  return (
    <section className="rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-4 shadow-lg">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        {hasFilters && (
          <button
            className="rounded border border-white/25 px-3 py-2 text-xs"
